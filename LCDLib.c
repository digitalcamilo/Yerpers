...
void LCD_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str, uint16_t Color)
{
    ...
    LCD_WriteReg(HOR_ADDR_START_POS, MIN_SCREEN_Y);     /* Horizontal GRAM Start Address */
    LCD_WriteReg(HOR_ADDR_END_POS, (MAX_SCREEN_Y - 1));  /* Horizontal GRAM End Address */
    LCD_WriteReg(VERT_ADDR_START_POS, MIN_SCREEN_X);    /* Vertical GRAM Start Address */
...
void LCD_SetPoint(uint16_t Xpos, uint16_t Ypos, uint16_t color)
{
    ...
    if (Xpos < MIN_SCREEN_X || Xpos > MAX_SCREEN_X || Ypos < MIN_SCREEN_Y || Ypos > MAX_SCREEN_Y)
        return;
...
uint16_t ReadPixelColor(uint16_t x, uint16_t y)
{
    LCD_SetCursor(x, y);
    LCD_WriteIndex(GRAM);
